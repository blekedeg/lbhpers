...
/**
 * Format number with thousand separator (number_format.js dependent)
 */
var thousandNumberFormat = {
	fields: document.getElementsByClassName('thousand-number'),
	init: function() {
		for (var i = 0; i < this.fields.length; i++) {
			// load default
			this.fields[i].value = number_format(this.fields[i].value, 0);
			this.fields[i].innerHTML = number_format(this.fields[i].innerHTML, 0);
			//this.fields[i].style.textAlign = 'right';
