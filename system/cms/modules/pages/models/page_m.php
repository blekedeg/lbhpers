...
class Page_m extends MY_Model
{
	...
	*/
	...
	public function hit_counter()
	{
		...
		$res = $this->db->query("SELECT * FROM ".$this->db->dbprefix('visitors_counter')." WHERE url = '".$url."' AND DATE(time_hit) = DATE(NOW())")->result_array();
		if ($res) {
			$this->db->query("UPDATE ".$this->db->dbprefix('visitors_counter')." SET counter = counter + 1, time_hit = NOW() WHERE url = '{$url}' AND DATE(time_hit) = DATE(NOW())");
		} else {
			$this->db->query("INSERT INTO ".$this->db->dbprefix('visitors_counter')." (counter, url, time_hit) VALUES (1, '{$url}', NOW())");
		}
	}
	
	public function get_hit_counter()
	{
		return $this->db
			->query("
				SELECT
					...
					-- this_week AS \"This Week\",
					-- last_week AS \"Last Week\",
					this_month AS \"Bulan ini\",
					-- last_month AS \"Last Month\",
					all_day AS \"All Day\"
				FROM (
					(SELECT SUM(counter) AS today FROM ".$this->db->dbprefix('visitors_counter')." WHERE DATE(time_hit) = DATE(NOW())) AS today,
					-- (SELECT SUM(counter) AS yesterday FROM ".$this->db->dbprefix('visitors_counter')." WHERE DATE(time_hit) = DATE(NOW() - INTERVAL 1 DAY)) AS yesterday,
					-- (SELECT SUM(counter) AS this_week FROM ".$this->db->dbprefix('visitors_counter')." WHERE WEEK(time_hit) = WEEK(NOW())) AS this_week,
					-- (SELECT SUM(counter) AS last_week FROM ".$this->db->dbprefix('visitors_counter')." WHERE WEEK(time_hit) = WEEK(NOW() - INTERVAL 1 WEEK)) AS last_week,
					(SELECT SUM(counter) AS this_month FROM ".$this->db->dbprefix('visitors_counter')." WHERE MONTH(time_hit) = MONTH(NOW())) AS this_month,
					-- (SELECT SUM(counter) AS last_month FROM ".$this->db->dbprefix('visitors_counter')." WHERE MONTH(time_hit) = MONTH(NOW() - INTERVAL 1 MONTH)) AS last_month,
					(SELECT SUM(counter) AS all_day FROM ".$this->db->dbprefix('visitors_counter').") AS all_day
				)
			")
			->row_array();
	}
	
	public function get_latest_catalogues($limit = 3)
	{
		return $this->db
			->query("SELECT * FROM ".$this->db->dbprefix('catalogue')." ORDER BY catalogue_id ASC LIMIT {$limit}")
			->result_array();
	}
}
