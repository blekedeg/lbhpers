...
class Events_Search
{
    ...
    public function __construct()
    {
		...
		// Post a news to twitter and whatnot
    ...
    public function index_post($id)
    {
    	...
    	if ($post->status === 'live')
    	{
    		$this->ci->search_index_m->index(
    			...
    			'news:posts', 
    			$id,
    			'news/'.date('Y/m/', $post->created_on).$post->slug,
    			$post->title,
    			$post->body, 
    			array(
    				'cp_edit_uri' 	=> 'admin/news/edit/'.$id,
    				'cp_delete_uri' => 'admin/news/delete/'.$id,
    	...
    	{
    		$this->ci->search_index_m->drop_index('news', 'news:post', $id);
    	}
	}
