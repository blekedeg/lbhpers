...
class Users extends Public_Controller
{
	...
	public function view($username = null)
	{
		...
		$this->template->build('profile/view', array(
			'_user' => $user,
			'page' => array('title' => $user->display_name)
		));
	}
	...
	public function login()
	{
		...
		$this->template
			->build('login', array(
				...
				'page' => array('title' => lang('user:login_header'))
			));
	}
	...
	public function register()
	{
		...
		$this->template
			->title(lang('user:register_title'))
			->set('_user', $user)
			->build('register', array('page' => array('title' => lang('user:register_header'))));
	}
	...
	public function activate($id = 0, $code = null)
	{
		...
		$this->template
			...
			->build('activate', array('page' => array('title' => 'Registrasi')));
	}
	...
	public function reset_pass($code = null)
	{
		...
		$this->template->build('reset_pass', array(
			'page' => array('title' => lang('user:reset_password_title'))
		));
	}
	...
	public function edit($id = 0)
	{
			...
			if ($this->ion_auth->update_user($user->id, $user_data, $profile_data) !== false)
			{
				...
				if ( ! empty($_FILES[$upload_key[0]]['name']) && $_FILES[$upload_key[0]]['error'] == UPLOAD_ERR_OK) {
					...
					if ( ! in_array($ext, $allowed)) {
						$json = array('error' => TRUE, 'message' => 'Invalid file type. Please upload file only type: .jpg, .jpeg');
					} else {
						
						$upload_dir = FCPATH.'data/profile/'.$user->user_id;
		
						if ( ! is_dir($upload_dir)) {
							mkdir($upload_dir, 0777);
						}
						if (@move_uploaded_file($tmp_name, $upload_dir.'/'.$file_name)) {
							$this->core->set['photo'] = $user->user_id.'/'.$file_name;
							$this->core->save('default_profiles', array('user_id' => $user->user_id));
						}
					}
				}
		...
		$this->template->build('profile/edit', array(
			...
			'page' => array('title' => ($this->current_user->id !== $user->id) ? sprintf(lang('user:edit_title'), $user->display_name) : lang('profile_edit'))
		));
	}
