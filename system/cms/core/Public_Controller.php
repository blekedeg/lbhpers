...
class Public_Controller extends MY_Controller
{
	...
	public function __construct()
	{
		...
		if (module_enabled('news'))
		{
			...
			$news_posts = $this->news_m->get_many_by(array('limit' => 10));
			foreach ($news_posts as &$post) {
				$this->news_m->process_post($post);
			}

			$this->template->set('news_posts', $news_posts);
		}

		if (module_enabled('program'))
		{
			$this->load->model('program/program_m');
		
			$program_posts = $this->program_m->get_many_by(array('limit' => 10));
			foreach ($program_posts as &$post) {
				$this->program_m->process_post($post);
			}

			$this->template->set('program_posts', $program_posts);
		}

		// hit counter
		//$this->page_m->hit_counter();
		
		//$page->hit_counter = $this->page_m->get_hit_counter();
