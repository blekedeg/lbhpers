 ...
 * @package PyroCMS\Core\Modules\Gallery\Controllers
 */
class Admin_Categories extends Admin_Controller
{
	...
	public function index()
	{
		...
		$total_rows = $this->gallery_categories_m->count_all();
		$pagination = create_pagination('admin/gallery/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->gallery_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->gallery_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new gallery category has been created.
				Events::trigger('gallery_category_created', $id);
		...
		$this->template
			...
			->set('mode', 'create')
			->append_js('module::gallery_category_form.js')
	...
	{
		// Get the category
		$category = $this->gallery_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/gallery/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->gallery_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A gallery category is being updated.
			Events::trigger('gallery_category_updated', $id);

			redirect('admin/gallery/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			Events::trigger('gallery_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->gallery_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->gallery_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->gallery_categories_m->insert_ajax($this->input->post());
