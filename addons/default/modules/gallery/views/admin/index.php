...
var dataList = {
				...
				for (var i = 0; i < json.data.length; i++) {
					...
					html += '\
						<tr>\
							...
							<td>' + json.data[i].title + '<br /><br /><a href="' + json.data[i].image + '" class="modal cboxElement"><img src="' + json.data[i].thumb + '" /></a></td>\
							<td>' + emptyNull(json.data[i].description) + '</td>\
							<td>' + status + '</td>\
							...
							<td width="150" nowrap="nowrap">\
								<a href="<?php echo site_url('admin/gallery/edit') ?>/' + json.data[i].id + '" class="button">Edit</a>\
								<a href="<?php echo site_url('admin/gallery/delete') ?>/' + json.data[i].id + '" class="button" onclick="return confirm(\'Are you sure you want to delete?\')">Delete</a>\
							</td>\
						</tr>\
					';
				}
	...
	search: function() {
		...
		this.url = '<?php echo site_url('admin/gallery') ?>';
		this.loadData();
