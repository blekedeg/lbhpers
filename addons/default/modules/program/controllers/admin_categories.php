 ...
 * @package PyroCMS\Core\Modules\Program\Controllers
 */
class Admin_Categories extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('program_categories_m');
	...
	public function index()
	{
		...
		$total_rows = $this->program_categories_m->count_all();
		$pagination = create_pagination('admin/program/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->program_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->program_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new program category has been created.
				Events::trigger('program_category_created', $id);
			...
			redirect('admin/program/categories');
		}
		...
		$this->template
			...
			->set('mode', 'create')
			->append_js('module::program_category_form.js')
	...
	{
		// Get the category
		$category = $this->program_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/program/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->program_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A program category is being updated.
			Events::trigger('program_category_updated', $id);

			redirect('admin/program/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->program_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('program_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->program_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->program_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->program_categories_m->insert_ajax($this->input->post());
