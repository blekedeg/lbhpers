...
class Program extends Public_Controller {
	...
	public function __construct()
	{
		...
		$cates = $this->db->get('program_categories')->result_array();
		...
		$this->stream = $this->streams_m->get_stream('program', true, 'programs');
	}
	...
	public function index()
	{
		...
		$params = array(
			...
			'where'			=> "`status` = 1",
			'paginate'		=> 'yes',
			'pag_base'		=> site_url('program/page'),
			'pag_segment'   => 3
		);
		...
		$this->template
			...
			->set_breadcrumb(lang('program:program_title'))
			->set_metadata('og:title', $this->module_details['name'], 'og')
			->set_metadata('og:type', 'program', 'og')
			...
			->build('posts', array('page' => array('title' => lang('program:program_title'))));
	}
	...
	{
		$slug or redirect('program');

		// Get category data
		$category = $this->program_categories_m->get_by('slug', $slug) OR show_404();

		// Get the program posts
		$params = array(
			'stream'		=> 'program',
			'namespace'		=> 'programs',
			'limit'			=> Settings::get('records_per_page'),
		...
		$this->template->title($this->module_details['name'], $category->title)
			...
			->set_breadcrumb(lang('program:program_title'), 'program')
	...
	private function _single_view($post)
	{
		...
		$this->template->set_breadcrumb(lang('program:program_title'), 'program');

		if ($post['category_id'] > 0)
		{
			// Get the category. We'll just do it ourselves
			// since we need an array.
			if ($category = $this->db->limit(1)->where('id', $post['category_id'])->get('program_categories')->row_array())
			{
				$this->template->set_breadcrumb($category['title'], 'program/category/'.$category['slug']);
		...
		$this->template
			->title($post['title'], lang('program:program_title'))
			...
			->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
			->set('post', array($post))
			->build('view', array('page' => array('title' => lang('program:program_title'))));
	}
	...
	private function _process_post(&$post)
	{
		...
		//$post['thumb_detail'] = $this->image_m->resize($image, 200, 150, 'crop');
		$post['thumb_detail'] = $this->image_m->resize($image, 940, 450, 'crop');
