 ...
 * @package PyroCMS\Core\Modules\Alert\Models
 */
class Alert_m extends MY_Model
{
	protected $_table = 'alert';
	...
	public function get_all()
	{
		$this->db
			...
			//->join('alert_categories', 'alert.category_id = alert_categories.id', 'left')
			->join('profiles', 'profiles.user_id = alert.author_id', 'left')
			->join('users', 'alert.author_id = users.id', 'left')
	...
	{
		return $this->db
			->select('alert.*, users.username, profiles.display_name')
			->join('profiles', 'profiles.user_id = alert.author_id', 'left')
			->join('users', 'alert.author_id = users.id', 'left')
			->where('alert.id', $id)
			->get('alert')
	...
	public function get_many_by($params = array())
	{
		/*if ( ! empty($params['category']))
		{
			...
			{
				$this->db->where('alert_categories.id', $params['category']);
			}
			else
			{
				$this->db->where('alert_categories.slug', $params['category']);
			}
		}*/

		if ( ! empty($params['month']))
		{
			$this->db->where('MONTH(FROM_UNIXTIME('.$this->db->dbprefix('alert').'.created_on))', $params['month']);
		}

		if ( ! empty($params['year']))
		{
			$this->db->where('YEAR(FROM_UNIXTIME('.$this->db->dbprefix('alert').'.created_on))', $params['year']);
		}

		if ( ! empty($params['keywords']))
		{
			$this->db
				->like('alert.title', trim($params['keywords']))
		...
		{
			$this->db->where('alert.created_on <=', now());
		}
	...
	public function count_tagged_by($tag, $params)
	{
		return $this->select('*')
			->from('alert')
			->join('keywords_applied', 'keywords_applied.hash = alert.keywords')
	...
	{
		return $this->db->select('alert.*, alert.title title, alert.slug slug, profiles.display_name')
			->from('alert')
			->join('keywords_applied', 'keywords_applied.hash = alert.keywords')
			->join('keywords', 'keywords.id = keywords_applied.keyword_id')
			//->join('alert_categories', 'alert_categories.id = alert.category_id', 'left')
			->join('profiles', 'profiles.user_id = alert.author_id', 'left')
			->where('keywords.name', str_replace('-', ' ', $tag))
			->where($params)
			->get()
	...
	{
		//$this->db->join('alert_categories', 'alert.category_id = alert_categories.id', 'left')
		// we need the display name joined so we can get an accurate count when searching
		$this->db->join('profiles', 'profiles.user_id = alert.author_id');
		...
		return $this->db->count_all_results('alert');
	}
	...
	public function get_archive_months()
	{
		...
		$this->db->from('alert t1');
		$this->db->distinct();
		$this->db->select('(SELECT count(id) FROM '.$this->db->dbprefix('alert').' t2
	...
	/**
	 * Searches alert posts based on supplied data array
	 *
	 * @param $data array
	 *
	 * @return array
	...
	public function search($data = array())
	{
		...
		if (array_key_exists('keywords', $data))
		{
			...
			foreach ($phrases as $phrase)
			{
				if ($counter == 0)
				{
					$this->db->like('alert.title', $phrase);
				}
				else
				{
					$this->db->or_like('alert.title', $phrase);
				}

				$this->db->or_like('alert.body', $phrase);
				$this->db->or_like('alert.intro', $phrase);
	...
	public function process_post(&$post)
	{
		...
		$post->url = site_url('alert/'.date('Y/m/d', $post->created_on).'/'.$post->slug);
