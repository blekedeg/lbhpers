...
class Admin extends Admin_Controller
{
	...
	private $_alert_base_file_dir;
	private $_alert_base_file_cache_dir;
	private $_alert_base_file_url;
	...
	protected $validation_rules = array(
		...
		/*array(
			'field' => 'category_id',
			'label' => 'lang:alert:category_label',
			'rules' => 'trim|numeric'
		),*/
		array(
			'field' => 'keywords',
		...
		array(
			'field' => 'intro',
			'label' => 'lang:alert:intro_label',
			'rules' => 'trim|required'
		),
		array(
			'field' => 'body',
			'label' => 'lang:alert:content_label',
			'rules' => 'trim|required'
		),
		array(
			'field' => 'type',
		...
		array(
			'field' => 'status',
			'label' => 'lang:alert:status_label',
			'rules' => 'trim|alpha'
		),
		array(
			'field' => 'created_on',
			'label' => 'lang:alert:date_label',
			'rules' => 'trim|required'
		),
		array(
			'field' => 'created_on_hour',
			'label' => 'lang:alert:created_hour',
			'rules' => 'trim|numeric|required'
		),
		array(
			'field' => 'created_on_minute',
			'label' => 'lang:alert:created_minute',
			'rules' => 'trim|numeric|required'
		),
		array(
			'field' => 'comments_enabled',
			'label' => 'lang:alert:comments_enabled_label',
			'rules' => 'trim|required'
		),
		array(
			'field' => 'preview_hash',
	...
	public function __construct()
	{
		...
		$this->load->model(array('alert_m'));
		$this->lang->load(array('alert'));

		$this->load->library(array('keywords/keywords', 'form_validation'));

		/*$_categories = array();
		if ($categories = $this->alert_categories_m->order_by('title')->get_all())
		{
		...
		$alert_dir = UPLOAD_PATH.'alert';
		if ( ! is_dir($alert_dir)) {
			mkdir($alert_dir, 0777);
		}
		
		$this->_alert_base_file_dir = $alert_dir;
		$this->_alert_base_file_cache_dir = $alert_dir;
		$this->_alert_base_file_url = base_url($alert_dir);
	...
	public function create()
	{
		...
		// Get the alert stream.
		$this->load->driver('Streams');
		$stream = $this->streams->streams->get_stream('alert', 'alert');
		$stream_fields = $this->streams_m->get_stream_fields($stream->id, $stream->stream_namespace);

		// Get the validation for our custom alert fields.
		$alert_validation = $this->streams->streams->validation_array($stream->stream_slug, $stream->stream_namespace, 'new');
		
		// Combine our validation rules.
		$rules = array_merge($this->validation_rules, $alert_validation);
		...
		if ($this->form_validation->run())
		{
			...
			if ($id = $this->streams->entries->insert_entry($_POST, 'alert', 'alert', array('created'), $extra))
			{
				// Upload image
				if ($_FILES) {
					$upload_key = array_keys($_FILES);
					...
					if ( ! empty($_FILES[$upload_key[0]]['name']) && $_FILES[$upload_key[0]]['error'] == UPLOAD_ERR_OK) {
						...
						if (@move_uploaded_file($tmp_name, $this->_alert_base_file_dir.'/'.$file_name)) {
							$this->db->update('alert', array('image' => $file_name), array('id' => $id));
						}
					}
				}

				$this->pyrocache->delete_all('alert_m');
				$this->session->set_flashdata('success', sprintf($this->lang->line('alert:post_add_success'), $this->input->post('title')));

				// alert article has been updated, may not be anything to do with publishing though
				Events::trigger('post_created', $id);

				// They are trying to put this live
				if ($this->input->post('status') == 'live')
				{
					// Fire an event, we're posting a new alert!
					Events::trigger('post_published', $id);
				}
			}
			else
			{
				$this->session->set_flashdata('error', lang('alert:post_add_error'));
			}

			// Redirect back to the form or main page
			($this->input->post('btnAction') == 'save_exit') ? redirect('admin/alert') : redirect('admin/alert/edit/'.$id);
		}
		else
		{
		...
		$this->template
			->title($this->module_details['name'], lang('alert:create_title'))
			->append_metadata($this->load->view('fragments/wysiwyg', array(), true))
			->append_js('jquery/jquery.tagsinput.js')
			->append_js('module::alert_form.js')
			->append_js('module::alert_category_form.js')
	 ...
	 * Edit alert post
	 *
	 * @param int $id The ID of the alert post to edit
	 */
	public function edit($id = 0)
	{
		$id or redirect('admin/alert');

		$post = $this->alert_m->get($id);
		...
		// Get the validation for our custom alert fields.
		$alert_validation = $this->streams->streams->validation_array($stream->stream_slug, $stream->stream_namespace, 'alert');
		
		$alert_validation = array_merge($this->validation_rules, array(
			'title' => array(
				'field' => 'title',
				'label' => 'lang:global:title',
		...
		$this->form_validation->set_rules(array_merge($this->validation_rules, $alert_validation));
		...
		if ($this->form_validation->run())
		{
			...
			if ($this->streams->entries->update_entry($id, $_POST, 'alert', 'alert', array('updated'), $extra))
			{
				// Upload image
				if ($_FILES) {
					$upload_key = array_keys($_FILES);
					...
					if ( ! empty($_FILES[$upload_key[0]]['name']) && $_FILES[$upload_key[0]]['error'] == UPLOAD_ERR_OK) {
						...
						if (@move_uploaded_file($tmp_name, $this->_alert_base_file_dir.'/'.$file_name)) {
							@unlink($this->_alert_base_file_dir.'/'.$post->image);
				...
				$this->session->set_flashdata(array('success' => sprintf(lang('alert:edit_success'), $this->input->post('title'))));
			...
			{
				$this->session->set_flashdata('error', lang('alert:edit_error'));
			}
		...
		if (file_exists(UPLOAD_PATH.'alert/'.$post->image) && is_file(UPLOAD_PATH.'alert/'.$post->image)) {
			$image = $this->_alert_base_file_dir.'/'.$post->image;

			$this->load->model('files/image_m');

			$post->image = $this->_alert_base_file_url.'/'.$post->image;
		...
		$this->template
			->title($this->module_details['name'], sprintf(lang('alert:edit_title'), $post->title))
			->append_metadata($this->load->view('fragments/wysiwyg', array(), true))
			->append_js('jquery/jquery.tagsinput.js')
			->append_js('module::alert_form.js')
	 ...
	 * Preview alert post
	 *
	 * @param int $id The ID of the alert post to preview
	 */
	 ...
	 * Publish alert post
	 *
	 * @param int $id the ID of the alert post to make public
	 */
	public function publish($id = 0)
	{
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->alert_m->get($id))
				{
					$this->alert_m->publish($id);

					// Wipe cache for this model, the content has changed
					$this->pyrocache->delete('alert_m');
		...
		if ( ! empty($post_titles))
		{
			...
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('alert:publish_success'), $post_titles[0]));
			}
			// Publishing multiple posts
			else
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('alert:mass_publish_success'), implode('", "', $post_titles)));
			}
		}
		// For some reason, none of them were published
		else
		{
			$this->session->set_flashdata('notice', $this->lang->line('alert:publish_error'));
		}
	 ...
	 * Delete alert post
	 *
	 * @param int $id The ID of the alert post to delete
	 */
	public function delete($id = 0)
	{
		$this->load->model('comments/comment_m');

		role_or_die('alert', 'delete_live');
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->alert_m->get($id))
				{
					if ($this->alert_m->delete($id))
					{
						$this->comment_m->where('module', 'alert')->delete_by('entry_id', $id);
			...
			// Fire an event. We've deleted one or more alert posts.
		...
		if ( ! empty($post_titles))
		{
			...
			if (count($post_titles) == 1)
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('alert:delete_success'), $post_titles[0]));
			}
			// Deleting multiple pages
			else
			{
				$this->session->set_flashdata('success', sprintf($this->lang->line('alert:mass_delete_success'), implode('", "', $post_titles)));
			}
		}
		// For some reason, none of them were deleted
		else
		{
			$this->session->set_flashdata('notice', lang('alert:delete_error'));
		}
	...
	{
		$this->form_validation->set_message('_check_title', sprintf(lang('alert:already_exist_error'), lang('global:title')));

		return $this->alert_m->check_exists('title', $title, $id);
	}
	...
	{
		$this->form_validation->set_message('_check_slug', sprintf(lang('alert:already_exist_error'), lang('global:slug')));

		return $this->alert_m->check_exists('slug', $slug, $id);
	}
	...
	public function _check_extension($ext = '')
	{
		$allowed = array(
			'.jpg',
			'.jpeg',
...
echo $ext;exit;
		if ( ! in_array($ext, $allowed)) {
			$this->form_validation->set_message('_check_extension', 'Invalid image file extension. Allowed extension are .jpg, .jpeg, .png, .gif');
			return FALSE;
		} else {
			return FALSE;
		}
