...
class Admin extends Admin_Controller
{
	...
	private $_news_base_file_dir;
	private $_news_base_file_cache_dir;
	private $_news_base_file_url;
	...
	public function __construct()
	{
		...
		$news_dir = UPLOAD_PATH.'news';
		if ( ! is_dir($news_dir)) {
			mkdir($news_dir, 0777);
		}
		
		$this->_news_base_file_dir = $news_dir;
		$this->_news_base_file_cache_dir = $news_dir;
		$this->_news_base_file_url = base_url($news_dir);
	...
	public function create()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if ($id = $this->streams->entries->insert_entry($_POST, 'news', 'news', array('created'), $extra))
			{
				// Upload image
				if ($_FILES) {
					$upload_key = array_keys($_FILES);
