...
class News extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->stream = $this->streams_m->get_stream('news', true, 'news');
	}
	...
	public function index()
	{
		...
		$this->template
			...
			->build('posts', array('page' => array('title' => lang('news:news_title'))));
	}
	...
	private function _process_post(&$post)
	{
		...
		$post['url'] = site_url('news/'.$post['id'].'/'.date('Y/m/d', $post['created_on']).'/'.$post['slug']);
	...
	private function _single_view($post)
	{
		...
		$this->template
			...
			->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
			->set('post', array($post))
			->build('view', array('page' => array('title' => lang('news:news_title'))));
	}
}
