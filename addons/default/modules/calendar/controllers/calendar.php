...
class Calendar extends Public_Controller {
    ...
    public function __construct()
    {
		...
		$this->_partials = $this->pyrocache->model('calendar_m', 'list_event_by');
  	...
  	function index()
  	{
		...
		$this->data->event_list = $this->_partials;
		$this->data->page = array('title' => lang('calendar_home_label'));
	...
	function detail($data_read = 0, $date_str = "")
	{
		...
		if($last_date_str !== false && $last_date_str == 16){
			$this->data->date_str = substr($date_str, 0, 10).' '.substr($date_str, 11, 2).':'.substr($date_str, 14, 2);
			$this->data->date_url = substr($date_str, 0, 16);
		}else{
			$this->data->date_str = "";
			$this->data->date_url = "";
		}
		...
		if (file_exists(UPLOAD_PATH.'calendar/'.$this->data->data_read->image) && is_file(UPLOAD_PATH.'calendar/'.$this->data->data_read->image)) {
			$image = UPLOAD_PATH.'calendar/'.$this->data->data_read->image;
		} else {
		...
		//$this->data->data_read->thumb_detail = $this->image_m->resize($image, 200, 150, 'crop');
		$this->data->data_read->thumb_detail = $this->image_m->resize($image, 616, 350, 'crop');	
 ...
 function _date($srctime)
 {
	...
	$months_name = array(0 => '', 1 => 'Januari', 2 => 'Februari', 3 => 'Maret', 4 => 'April', 5 => 'Mei', 6 => 'Juni', 7 => 'Juli', 8 => 'Agustus', 9 => 'September', 10 => 'Oktober', 11 => 'Nopember', 12 => 'Desember');
	...
	$this->data->current_month_text = $months_name[$current_month].' '.$current_year;
