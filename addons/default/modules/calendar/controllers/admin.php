...
class Admin extends Admin_Controller {
	...
	private $data;
	private $_calendar_base_file_dir;
	private $_calendar_base_file_cache_dir;
	private $_calendar_base_file_url;
    ...
    public function __construct()
    {
        ...
        $calendar_dir = UPLOAD_PATH.'calendar';
		if ( ! is_dir($calendar_dir)) {
			mkdir($calendar_dir, 0777);
		}
		
		$this->_calendar_base_file_dir = $calendar_dir;
		$this->_calendar_base_file_cache_dir = $calendar_dir;
		$this->_calendar_base_file_url = base_url($calendar_dir);
    }
    ...
    function create($date_begin = ''){
		...
		if ($this->form_validation->run())
		{
            ...
            $repeat_prm = array(
				...
				'minute'=> $this->input->post('repeat_minute'),
			...
			if ($id)
			{
				...
				if ($_FILES) {
					...
					if ( ! empty($_FILES[$upload_key[0]]['name']) && $_FILES[$upload_key[0]]['error'] == UPLOAD_ERR_OK) {
						...
						if (@move_uploaded_file($tmp_name, $this->_calendar_base_file_dir.'/'.$file_name)) {
							@unlink($this->_calendar_base_file_dir.'/'.$post->image);

							$this->db->update('eventcal', array('image' => $file_name), array('id_eventcal' => $id));
						}
					}
				}
			}
  ...
  function edit($id=0){
        ...
        if (file_exists(UPLOAD_PATH.'calendar/'.$post->image) && is_file(UPLOAD_PATH.'calendar/'.$post->image)) {
			$image = $this->_calendar_base_file_dir.'/'.$post->image;

			$this->load->model('files/image_m');

			$post->image = $this->_calendar_base_file_url.'/'.$post->image;
