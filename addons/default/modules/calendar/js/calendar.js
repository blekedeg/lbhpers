...
//configure the date format to match mysql date
	...
	$('#'+$('select[name="repeat_minute"]').attr('id')+'_chzn').show();
	$('._minute_label').css('display','inline-block');
	...
	var initFormInput = function(){
		...
		if (ob_chzn != undefined && frmInputCounter < 5) {
			...
			var serem = $('#'+$('select[name="repeat_minute"]').attr('id')+'_chzn');
			serem.show();
			serem.before('<div class="repeat_label _minute_label" style="display:inline-block;">'+$('select[name="repeat_minute"]').attr("title")+'</div>');
