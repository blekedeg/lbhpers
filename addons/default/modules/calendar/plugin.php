...
class Plugin_Calendar extends Plugin
{
	...
	public function read()
	{
        ...
        if(!$posts = $this->pyrocache->get('calendar_cache'.DIRECTORY_SEPARATOR.'cache_'.$vname))
        {
            ...
            $posts1 = $this->db
			->select($tbl.'.id_eventcal, '.$tbl.'.event_title as title, '.$tbl.'.event_date_begin as date_start, '.$tbl.'.event_date_end as date_end, '.$tjoin.'.display_name as user_post, '.$tbl.'.image')
			...
			if ($repeated == 'yes') {
				$posts2 = $this->db
				->select($tbl.'.id_eventcal, '.$tbl.'.event_title as title, '.$tbl.'.event_date_begin as date_start, '.$tbl.'.event_date_end as date_end, '.$tjoin.'.display_name as user_post, '.$tbl.'.event_repeat_prm, '.$tbl.'.image')
				->join($tjoin, $tbl.'.user_id = '.$tjoin.'.user_id', 'left')
				...
				foreach($posts2 as $key => $row_event){
					...
					if(!isset($prm->minute)){$prm->minute = "00";}
					if(isset($prm->type) and $prm->type == 0){
						if(!empty($repset) and !empty($repset['daily'])){
							...
							if($count_d_p > 0){
								for($cd = 0; $cd < $count_d_p; $cd++){
									$xday = $count_d_p - $cd;
									$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00 -'.$xday.' day'));
									$represult[] = $this->__setEventRow($row_event, $row_event->event_date_begin);
								}
							}
							if($count_d_n > 0){
								for($cd = 0; $cd < $count_d_n; $cd++){
									$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00 +'.$cd.' day'));
									$represult[] = $this->__setEventRow($row_event, $row_event->event_date_begin);
								}
							}

						}else{
							$event_time = strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00');
							if($curtime > $event_time){
								$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00 +1 day'));
						...
						for($k = 0; $k < 7; $k++){
							...
							if(date('w', $looptime) == $prm->day){
								$event_time = strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00'.' +'.$k.' day');
								if($curtime <= $event_time){
									$row_event->event_date_begin = date('Y-m-d H:i:s', $event_time);
									break;
								}else{
									$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime(date('Y-m-d').' '.$prm->time.':'.$prm->minute.':00'.' +'.($k+7).' day'));
									break;
								}
							}
						}
						...
						if(!empty($repset) and !empty($repset['weekly'])){
							...
							if($count_d_p > 0){
								for($cd = 0; $cd < $count_d_p; $cd++){
									$xday = ($count_d_p - $cd) * 7;
									$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime($curdate.' '.$prm->time.':'.$prm->minute.':00 -'.$xday.' day'));
						...
						$event_time = strtotime(date('Y-m-').$prm->date.' '.$prm->time.':'.$prm->minute.':00');
						$lasmonthtime = strtotime(date('Y-m-').'01 23:00:00 +1 month -1 day');
						if($event_time <= $lasmonthtime){
							$row_event->event_date_begin = date('Y-m-d H:i:s', $event_time);
						}elseif(!$thismonth){
							$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime(date('Y-m-').$prm->date.' '.$prm->time.':'.$prm->minute.':00 +1 month'));
						}
						...
						if(!empty($repset) and !empty($repset['monthly'])){
							...
							if($count_d_p > 0){
								for($cd = 0; $cd < $count_d_p; $cd++){
									$xday = $count_d_p - $cd;
									$row_event->event_date_begin = date('Y-m-d H:i:s', strtotime($curdate.' '.$prm->time.':'.$prm->minute.':00 -'.$xday.' month'));
									$represult[] = $this->__setEventRow($row_event, $row_event->event_date_begin);
								}
							}
							if($count_d_n > 0){
		...
		foreach ($posts as &$post)
		{
			...
			$post->f_date_start = indonesian_date($post->date_start);
			$post->f_date_end = empty($post->date_end) ? "" : date($dateformat, strtotime($post->date_end));

			if (file_exists(UPLOAD_PATH.'calendar/'.$post->image) && is_file(UPLOAD_PATH.'calendar/'.$post->image)) {
				$image = UPLOAD_PATH.'calendar/'.$post->image;
			} else {
