...
class Pengaduan_m extends MY_Model {
	...
	public function get_many_by($params = array())
	{
		...
		{
			$this->db->where('MONTH(FROM_UNIXTIME('.$this->db->dbprefix('pengaduan').'.created_on))', $params['month']);
		}

		if ( ! empty($params['year']))
		{
			$this->db->where('YEAR(FROM_UNIXTIME('.$this->db->dbprefix('pengaduan').'.created_on))', $params['year']);
		}

		if ( ! empty($params['keywords']))
		{
			$this->db
				->like('pengaduan.subject', trim($params['keywords']));
		}
		...
		{
			$this->db->where('pengaduan.created_on <=', date('Y-m-d H:i:s', now()));
		}
	...
	public function process_post(&$post)
	{
		...
		//$post->url = site_url('pengaduan/'.date('Y/m/d', $post->created_on).'/'.$post->slug);
		$slug = url_title($post->title, 'dash', TRUE);
		$post->url = site_url('pengaduan/'.date('Y/m/d', strtotime($post->created_on)).'/'.$slug);
