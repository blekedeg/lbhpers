...
class Admin extends Admin_Controller {
	...
	protected $section = 'kasus';

	private $_kasus_base_file_dir;
	private $_kasus_base_file_cache_dir;
	private $_kasus_base_file_url;
	...
	public function __construct()
	{
		...
		$this->lang->load(array('kasus', 'categories'));

		$this->load->model(array('kasus_m', 'kasus_categories_m'));

		$kasus_dir = UPLOAD_PATH.'kasus';
		if ( ! is_dir($kasus_dir)) {
			mkdir($kasus_dir, 0777);
		}
		
		$this->_kasus_base_file_dir = $kasus_dir;
		$this->_kasus_base_file_cache_dir = $kasus_dir;
		$this->_kasus_base_file_url = base_url($kasus_dir);

		$_categories = array();
		if ($categories = $this->kasus_categories_m->order_by('title')->get_all())
		{
		...
		if ($this->core->is_ajax()) {
			...
			if ($this->input->get('cmd') == 'load_data_list') {
				...
				$sql = "
					...
					FROM ".$this->db->dbprefix('kasus')." prg
				...
				$config['base_url'] = site_url('/admin/kasus/index');
	...
	public function add()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($id = $this->streams->entries->insert_entry($_POST, 'kasus', 'kasus', array('created'), $extra))
			{
				// Upload image
				if ($_FILES) {
					$upload_key = array_keys($_FILES);
					...
					if ( ! empty($_FILES[$upload_key[0]]['name']) && $_FILES[$upload_key[0]]['error'] == UPLOAD_ERR_OK) {
						...
						if (@move_uploaded_file($tmp_name, $this->_kasus_base_file_dir.'/'.$file_name)) {
							@unlink($this->_kasus_base_file_dir.'/'.$post->image);

							$this->db->update('kasus', array('image' => $file_name), array('id' => $id));
						}
					}
				}

				$this->pyrocache->delete_all('kasus_m');
				$this->session->set_flashdata('success', sprintf($this->lang->line('kasus:post_add_success'), $this->input->post('title')));
				...
				{
					// Fire an event, we're posting a new kasus!
					Events::trigger('post_published', $id);
				}
			}
			else
			{
				$this->session->set_flashdata('error', lang('kasus:post_add_error'));
			}

			// Redirect back to the form or main page
			($this->input->post('btnAction') == 'save_exit') ? redirect('admin/kasus') : redirect('admin/kasus/edit/'.$id);
		}
	...
	public function edit($id = 0)
	{
		...
		if ($this->form_validation->run()) {
			...
			$this->db->update('kasus', $data, array('id' => $id));
		...
		if (file_exists(UPLOAD_PATH.'kasus/'.$post->image) && is_file(UPLOAD_PATH.'kasus/'.$post->image)) {
			$image = $this->_kasus_base_file_dir.'/'.$post->image;

			$this->load->model('files/image_m');

			$post->image = $this->_kasus_base_file_url.'/'.$post->image;
	...
	public function delete($id = 0)
	{
		...
		$this->session->set_flashdata('success', sprintf($this->lang->line('kasus:delete_success'), $post->title));
