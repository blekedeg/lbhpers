...
class Admin_Categories extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('kasus_categories_m');
	...
	public function index()
	{
		...
		$total_rows = $this->kasus_categories_m->count_all();
		$pagination = create_pagination('admin/kasus/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->kasus_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->kasus_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new kasus category has been created.
				Events::trigger('kasus_category_created', $id);
			...
			redirect('admin/kasus/categories');
		}
		...
		$this->template
			...
			->set('mode', 'create')
			->append_js('module::kasus_category_form.js')
	...
	{
		// Get the category
		$category = $this->kasus_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/kasus/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->kasus_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A kasus category is being updated.
			Events::trigger('kasus_category_updated', $id);

			redirect('admin/kasus/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->kasus_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('kasus_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->kasus_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->kasus_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->kasus_categories_m->insert_ajax($this->input->post());
