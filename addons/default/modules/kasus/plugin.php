 ...
 * @package		PyroCMS\Addons\Default\Modules\Plugins
 */
class Plugin_Kasus extends Plugin 
{
	...
	/*public function posts()
	{
		...
		$rows = $this->program_m->get_all();

		foreach ($rows as &$row) {
			//$row->url = site_url('program/show/'.$this->core->encode($row->id).'/'.date('Y/m/d', strtotime($row->created_on)).'/'.url_title($row->title, 'dash', TRUE));
			$row->date = indonesian_date($row->created_on);

			if (file_exists(UPLOAD_PATH.'program/'.$row->image) && is_file(UPLOAD_PATH.'program/'.$row->image)) {
				$image = UPLOAD_PATH.'program/'.$row->image;
			} else {
				$image = UPLOAD_PATH.'no_image.jpg';
			}

			$row->thumb_70x70 = $this->image_m->resize($image, 70, 70, 'crop');
		}
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('kasus_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('kasus_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('kasus_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('kasus_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('kasus_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('kasus_categories', true).' ON '.$this->db->protect_identifiers('kasus_categories.id', true).' = '.$this->db->protect_identifiers('kasus.category_id', true);
		...
		{		
			...
			foreach ($posts['entries'] as &$post)
			{
				...
				$post['url'] = site_url('kasus/'.date('Y/m/d', strtotime($post['created_on'])).'/'.$post['slug']);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'kasus', 'kasus', $loop);
	}
