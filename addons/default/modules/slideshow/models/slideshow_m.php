...
class Slideshow_m extends MY_Model {
	...
	public function get_many_by($params = array())
	{
		...
		{
			$this->db->where('MONTH(FROM_UNIXTIME('.$this->db->dbprefix('download').'.created_on))', $params['month']);
		}

		if ( ! empty($params['year']))
		{
			$this->db->where('YEAR(FROM_UNIXTIME('.$this->db->dbprefix('download').'.created_on))', $params['year']);
		}

		if ( ! empty($params['keywords']))
		{
			$this->db
				->like('download.title', trim($params['keywords']));
		}
		...
		{
			$this->db->where('download.created_on <=', date('Y-m-d H:i:s', now()));
		}
