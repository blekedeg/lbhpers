 ...
 * @package		PyroCMS\Core\Modules\Slideshow\Plugins
 */
class Plugin_Slideshow extends Plugin
{
	...
	public $description = array(
		'en' => 'Displays download for site visitors.',
	...
	public function posts()
	{
		...
		$rows = $this->slideshow_m->get_all();

		foreach ($rows as &$row) {
			if (file_exists(UPLOAD_PATH.'slideshow/'.$row->image) && is_file(UPLOAD_PATH.'slideshow/'.$row->image)) {
				$image = UPLOAD_PATH.'slideshow/'.$row->image;
			} else {
				$image = UPLOAD_PATH.'no_image.jpg';
			}
			
			$row->image = $this->image_m->resize($image, 485, 344, 'crop');
		}
