...
class Plugin_Videos extends Plugin 
{
	...
	public $description = array(
		'en' => 'Displays videos for site visitors.',
	...
	public function posts()
	{
		...
		$rows = $this->videos_m->get_all();

		foreach ($rows as &$row) {
			$row->url = site_url('videos/view/'.$this->core->encode($row->id));
			$row->date = indonesian_date($row->created_on); 
			/*
			if (file_exists(UPLOAD_PATH.'videos/'.$row->image) && is_file(UPLOAD_PATH.'videos/'.$row->image)) {
				$image = UPLOAD_PATH.'videos/'.$row->image;
			} else {
